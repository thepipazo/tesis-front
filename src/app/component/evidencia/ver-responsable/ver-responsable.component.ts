...
@Component({
  selector: 'app-ver-responsable',
  templateUrl: './ver-responsable.component.html',
  styleUrls: ['./ver-responsable.component.css']
})
export class VerResponsableComponent implements OnInit {
  bloquearParteDelUsuario: boolean = false;
  ...
  ngOnInit(): void {
    ...
    this.permisoResponsable = false;
    this.permisoUsuario = false;
    this.permisoDca = false;
    this.permisoDirector = false;
  ...
  VerEvidencia(evidencia: number): void {
    this.evidenciaServicie.buscarPorId(evidencia).subscribe(resp => {
      ...
      this.bloquearParteDelUsuario = true;
      this.folio = resp.codigo;
      this.fecha = resp.fecha;
      this.estado = resp.estadoResponsable;


      if(resp.estadoResponsable == "En Espera"){
        this.onlyRead = false;
      }else{
        this.onlyRead = true;
  ...
  aprobarResponsable(): void {
    this.evidencia = new Evidencia(this.id, this.folio, this.descripcion, this.resultado, this.almacenamiento, this.presonaRelacionada, this.palabraClave, this.nombreCorto, 'Aprobado Por Responsable',
      ...
      this.ambitoGeografico, this.proceso, this.criterio, this.numeroPlan, 'Aprobado', 'En Espera',this.autoridadesI,this.administrativosI,this.docentesI,this.estudiantesI,this.autoridadesE,
    ...
    this.evidenciaServicie.actualizar(this.evidencia).subscribe(resp => {
      ...
      this.router.navigate(['/evidencia-responsable']);

    });
  }

  RechazarResponsable(): void {
    if(this.observacionResponsable == null){
