...
estadoReal:string;
  ...
  VerEvidencia(evidencia: number): void {
    ...
    this.evidenciaServicie.buscarPorId(evidencia).subscribe(resp => {
      ...
      this.estadoReal = resp.estado;



      if (resp.estadoDac == "En Espera") {
        if (resp.estadoResponsable == "Rechazado") {
          this.estado = "Rechazado";
        }

        if (resp.estadoResponsable == "Aprobado") {
          this.estado = "Aprobado Por Responsable";
        }
      } else {

        if (resp.estadoDac == "Rechazado") {
          this.estado = "Rechazado";
        }

        if (resp.estadoDac == "Aprobado") {
          this.estado = "Aprobado Por DCA";
        }
  ...
  async pdf() {
    ...
    pdf.add('Estado : ' + this.estadoReal)
    pdf.add(
      new Table([
